...
/**
 * Función de validación para reviews según la documentación oficial
 * Valida rating y comment antes de enviar al backend
 * 
 * @param rating - Calificación (1-5)
 * @param comment - Comentario opcional
 * @returns Objeto con isValid y array de errores
 */
export function validateReview(rating: number, comment?: string | null): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
/**
 * Función helper para manejar errores de peticiones HTTP según la documentación
 * Proporciona mensajes de error descriptivos basados en el código de estado
 * 
 ...
 */
function handleReviewError(error: any, defaultMessage: string = 'Error en la petición'): string {
  ...
  // Manejar diferentes códigos de error según la documentación
  if (errorMessage.includes('400') || errorMessage.includes('Bad Request')) {
    if (errorMessage.includes('comment')) {
      return 'El comentario puede estar causando un error. Verifica que tenga entre 10 y 1000 caracteres si lo incluyes.';
    } else if (errorMessage.includes('propertyId')) {
      return 'Error con el ID de la propiedad. Verifica que la propiedad existe.';
    } else if (errorMessage.includes('rating')) {
      return 'Error con la calificación. Verifica que sea un número entre 1 y 5.';
    }
    return 'Datos inválidos o faltantes. Verifica los datos enviados.';
  }
  ...
  if (errorMessage.includes('500') || errorMessage.includes('Internal Server Error')) {
    return 'Error del servidor. Por favor, intenta más tarde o contacta al soporte.';
  }
...
export const reviewService = {
   ...
   */
  async getPropertyStats(propertyId: string): Promise<{ averageRating: number; totalReviews: number }> {
    try {
      console.log('🔍 [reviewService] Obteniendo estadísticas para propiedad:', propertyId);
      
      const endpoint = `/api/reviews/property/${propertyId}/stats`;
      const response = await apiClient.get<any>(endpoint);
      ...
      if (response.success && response.data) {
        ...
        console.log(`✅ [reviewService] Estadísticas obtenidas: Rating ${stats.averageRating}, Total: ${stats.totalReviews}`);
        return stats;
      }
      
      // Si el endpoint no existe o falla, devolver valores por defecto
      console.warn('⚠️ [reviewService] No se pudieron obtener estadísticas, usando valores por defecto');
      return { averageRating: 0, totalReviews: 0 };
    } catch (error) {
      console.error('💥 [reviewService] Error obteniendo estadísticas:', error);
      return { averageRating: 0, totalReviews: 0 };
    }
  },
   ...
   * @returns Promise<Review[]> - Lista de reviews del usuario
   */
  async getUserReviews(userId: string): Promise<Review[]> {
    try {
      console.log('🔍 [reviewService] Obteniendo reviews del usuario:', userId);
      ...
      if (response.success) {
        ...
        if (Array.isArray(response.data)) {
          console.log(`✅ [reviewService] Reviews del usuario obtenidas: ${response.data.length}`);
          return response.data;
        }
        // Formato 2: { success: true, data: { reviews: [...] } }
        if (response.data?.reviews && Array.isArray(response.data.reviews)) {
          console.log(`✅ [reviewService] Reviews del usuario obtenidas: ${response.data.reviews.length}`);
          return response.data.reviews;
        }
        // Formato 3: Array directo
        if (Array.isArray(response)) {
          console.log(`✅ [reviewService] Reviews del usuario obtenidas: ${response.length}`);
          return response;
        }
      }
      
      console.warn('⚠️ [reviewService] Respuesta sin formato esperado, devolviendo array vacío');
      return [];
    } catch (error) {
      console.error('💥 [reviewService] Error obteniendo reviews del usuario:', error);
      
      if (error instanceof Error) {
        if (error.message.includes('401') || error.message.includes('Unauthorized')) {
          throw new Error('Debes iniciar sesión para ver tus reseñas');
        }
        
        if (error.message.includes('403') || error.message.includes('Forbidden')) {
          throw new Error('No tienes permisos para ver estas reseñas');
        }
      }
