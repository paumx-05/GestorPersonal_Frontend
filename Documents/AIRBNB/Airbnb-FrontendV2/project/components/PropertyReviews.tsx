...
const PropertyReviews = ({ propertyId, propertyRating, hostId, onRatingUpdate }: PropertyReviewsProps) => {
  ...
  // Determinar si el usuario puede crear reviews (no es propietario Y no tiene review existente)
  const canCreateReview = authIsAuthenticated && !isOwner && !existingReview;
    ...
    const loadReviews = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          // Verificar si el usuario actual ya tiene una review para esta propiedad
          // Según la guía: "No puedes crear más de una review por propiedad"
          if (user?.id && response.data.reviews) {
            const userReview = response.data.reviews.find(
              (review: Review) => review.userId === user.id
            );
            if (userReview) {
              setExistingReview(userReview);
              console.log('✅ [PropertyReviews] Usuario ya tiene una review para esta propiedad');
            } else {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setExistingReview(null); // Limpiar review existente ya que se creó una nueva
        ...
        if (updatedResponse.success && updatedResponse.data) {
          ...
          // Verificar si el usuario ahora tiene una review
          if (user?.id && updatedResponse.data.reviews) {
            const userReview = updatedResponse.data.reviews.find(
              (review: Review) => review.userId === user.id
            );
            if (userReview) {
              setExistingReview(userReview);
  ...
  const handleDeleteReview = async (reviewId: string) => {
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ [PropertyReviews] Review eliminada exitosamente');
        
        // Si era la review del usuario actual, limpiar el estado
        if (existingReview && existingReview.id === reviewId) {
          setExistingReview(null);
        }
        
        // Recargar lista de reviews y actualizar estadísticas
        setPage(1);
        const filters: ReviewFilters = { page: 1, limit, sort: 'newest' };
        const updatedResponse = await reviewService.getReviews(propertyId, filters);
        ...
        if (updatedResponse.success && updatedResponse.data) {
          ...
          // Verificar si el usuario aún tiene una review
          if (user?.id && updatedResponse.data.reviews) {
            const userReview = updatedResponse.data.reviews.find(
              (review: Review) => review.userId === user.id
            );
      ...
      console.error('💥 [PropertyReviews] Error eliminando review:', error);
  ...
  return (
    <div className="space-y-6 py-8 border-t border-gray-200">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Mensaje si el usuario ya tiene una review */}
        {existingReview && authIsAuthenticated && !isOwner && (
            ...
            <div key={review.id} className="border-b border-gray-200 pb-6 last:border-0">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* Botones de acción (solo si es la review del usuario actual) */}
